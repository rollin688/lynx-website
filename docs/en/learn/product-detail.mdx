---
description: 'Tutorial: implementing a high-performance swiper component in Lynx.'
sidebar: false
---

import { Go, Details, LearnTutorialTabs } from '@lynx';

export const swipeDemo = {
  // The web preview is driven by a mouse and this swiper by touch, so without
  // help it looks like a still image to a desktop reader. These synthesized
  // touches replay the swipe — walking to the last image and back — with the
  // contact point drawn the way a device simulator does it. The first real
  // touch or pointer hands the example back.
  loop: true,
  startDelayMs: 1400,
  steps: [
    {
...
<LearnTutorialTabs active="product-detail" />
...
- [Use Main Thread Script to Reduce Latency](#use-main-thread-script-to-reduce-latency): You'll learn how to optimize interaction performance with main thread script
...
Let's have a look at what we're building! To try it out, download and install the [Lynx Explorer App](/guide/start/quick-start.html#download-lynx-explorer,ios-simulator-platform=macos-arm64,explorer-platform=ios-simulator), then scan the QR code below.

The swiper responds to touch, so the web preview below demonstrates it for you: the swipes are simulated, and the circle marks where a finger would be. Touch or click the preview at any time and it hands control straight back to you.
...
Next, we use [Node Manipulation](/guide/interaction/event-handling/manipulating-element.react) to get the `swiper-container` node and use [`setNativeProps`](/api/lynx-api/nodes-ref/nodes-ref-set-native-props) to update the `transform` property, thereby updating the scroll position.
...
You can refer to [Direct Node Manipulation](/guide/interaction/event-handling/manipulating-element.react) to learn more.
:::
...
1. Encapsulate the [touch event listening](#listen-to-touch-events) code into `useOffset`, centralizing all scroll-related logic in this hook
...
We can use [Main Thread Script](/react/main-thread-script) to optimize this issue. After converting to main thread script, the scrolling becomes much smoother!
...
The main thread node provides many capabilities, as shown in [`MainThread.Element`](/api/lynx-api/main-thread/main-thread-element). Here we call the `setStyleProperties` method to modify the `transform` property, updating the `<Swiper>` component's position.
...
- From background to main thread: Use [`runOnMainThread`](/api/react/functions/runOnMainThread)
- From main thread to background: Use [`runOnBackground`](/api/react/functions/runOnBackground)
