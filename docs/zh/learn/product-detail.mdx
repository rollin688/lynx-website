...
让我们先看看这个应用的最终效果！要体验实际效果，请先下载安装 [Lynx Explorer App](/guide/start/quick-start.html#download-lynx-explorer,ios-simulator-platform=macos-arm64,explorer-platform=ios-simulator)，然后用它扫描下面的二维码。
...
这个教程中将展示很多漂亮图片，我们为你准备了一些基准图片，你可以在[这里](https://lf-lynx.tiktok-cdns.com/obj/lynx-artifacts-oss-sg/lynx-website/download/product_detail_assets.zip)下载资源包，并在搭建中使用它们。

## 直接操作节点

这里是一个产品详情页的实例，它包含一个轮播图和一些商品信息。其中，轮播图接受一个图片列表，并将其平铺展示。现在它没有办法滚动，让我们来让它动起来。

下面的预览同样会被自动滑动，但画面不会动——这正是重点，此时还没有任何代码在监听手势。
...
接下来，我们使用[操作节点](/guide/interaction/event-handling/manipulating-element.react)获取 `swiper-container` 的节点，并使用 [`setNativeProps`](/api/lynx-api/nodes-ref/nodes-ref-set-native-props) 更新 `transform` 属性，从而更新滚动位置。
...
你可以参考[直接操作节点](/guide/interaction/event-handling/manipulating-element.react)了解更多。
:::
...
我们可以使用[主线程脚本](/react/main-thread-script)来优化这个问题。可以看到，改造成主线程脚本后，滚动变得更加跟手了！
...
主线程获取的节点提供了很多能力，可以参考 [`MainThread.Element`](/api/lynx-api/main-thread/main-thread-element)。这里我们调用 `setStyleProperties` 方法修改 `transform` 属性，更新 `<Swiper>` 组件的位置。
...
- 从后台到主线程: 使用 [`runOnMainThread`](/api/react/functions/runOnMainThread)
- 从主线程到后台: 使用 [`runOnBackground`](/api/react/functions/runOnBackground)
