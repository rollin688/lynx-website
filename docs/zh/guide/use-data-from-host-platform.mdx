...
你可以在后续需要的时机通过 [`LynxView.updateData()`] (或 HarmonyOS 平台的 [`LynxContext.updateData()`]) 来更新这些数据：
...
你（或宿主平台开发人员）还可以在调用 [`LynxView.updateData()`] (或 HarmonyOS 平台的 [`LynxContext.updateData()`]) 时指定 `processorName`：
