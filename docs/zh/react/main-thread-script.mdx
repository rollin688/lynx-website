...
到目前为止的示例都将 `'main thread'` 函数用作事件处理器。但如果你需要从后台线程调用主线程函数，或反过来呢？ReactLynx 提供了 [`runOnMainThread()`] 和 [`runOnBackground()`] 来实现双向异步通信。
...
在这个示例中，点击方块会触发一个往返调用：

1. **主线程** `onTap` 触发，调用 `runOnBackground(incrementCount)()` 来更新状态
2. **后台线程** `count` 的 `useEffect` 触发，调用 `runOnMainThread(applyColor)(nextColor)` 来改变方块颜色
...
默认情况下，主线程函数无法直接调用没有 `'main thread'` 指令的普通函数。这使得代码复用变得困难。
为了解决这一限制，ReactLynx 引入了 `shared-module` 机制，允许显式声明某些模块可以在主线程和后台线程之间共享。

在 `import` 语句后添加 `with { runtime: 'shared' }`，该模块中的导出变量（包括函数、类、对象等）即可在主线程函数中被直接调用。
...
> [!NOTE]
> 以 `with { runtime: 'shared' }` 引入的模块中的函数虽然能够被主线程函数调用，但其本身并不会被自动识别为主线程函数。如果其中使用了 `MainThreadRef` 等主线程特有能力，仍需手动标记 `'main thread'`。
...
通常第三方库（例如 `motion-dom`）包含的都是普通 JavaScript 函数。如果直接在主线程函数中调用它们，会因为缺少 `'main thread'` 指令而报错。

我们可以利用 Shared Runtime 机制将它们作为共享模块引入。
...
为了方便复用，或者避开 Shared Runtime 的[限制](#限制)（例如赋值给新变量后失效），我们推荐**封装**一层主线程函数：
...
这样，`animate` 就变成了一个标准的主线程函数，可以在任何主线程函数中自由使用，不再受限于静态分析。
...
只有通过 `import` 直接引入的标识符才会被识别为“共享”。赋值给新变量后将失去共享特性。
...
**Shared Module 中的变量和状态在两个线程中是完全隔离的，各自拥有独立的实例。**

这意味着如果你在主线程修改了共享模块中的某个变量，后台线程是无法感知的，反之亦然。Shared Module 仅解决“代码共享”问题，不解决“状态共享”问题。

[`MainThread.Element`]: /api/lynx-api/main-thread/main-thread-element
[`MainThreadRef`]: /api/react/types/MainThreadRef
[`useMainThreadRef()`]: /api/react/hooks/useMainThreadRef
[`runOnMainThread()`]: /api/react/functions/runOnMainThread
[`runOnBackground()`]: /api/react/functions/runOnBackground
